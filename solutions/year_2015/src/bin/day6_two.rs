use std::{env, error::Error};

use input_file_lib::get_file_content_to_string;

fn main() -> Result<(), Box<dyn Error>> {
    //println!("there");
    let args: Vec<String> = env::args().collect();
    if args.len() != 2 {
        println!("Usage: day6 <input_file>");
	    std::process::exit(1);
    }

    let string_to_compute = get_file_content_to_string(&args[1])?;
    let mut grid: Vec<Vec<u32>> = vec![vec![0; 1000]; 1000];
    
    for line in string_to_compute.lines() {
        let instruction: Vec<&str> = line.split(' ').collect();
        //println!("{:?}", instruction);
        if (instruction.len() != 5) & (instruction.len() != 4) {
            //println!("there");
            continue;
        }
        match instruction[0] {
            "turn" => {
                match instruction[1] {
                    "on" => {
                        turn_on(&mut grid, &instruction[2], &instruction[4])
                    },
                    "off" => {
                        turn_off(&mut grid, &instruction[2], &instruction[4])
                    },
                    _ => {continue;}
                }
            },
            "toggle" => {
                toggle(&mut grid, &instruction[1], &instruction[3]);
            },
            _ => {continue;}
        }
    }
    
    let mut count_brightness: u32 = 0;
    for line in grid {
        count_brightness += count(line);
    }

    println!("{} total brightness", count_brightness);

    Ok(())
}


fn turn_on(grid: &mut Vec<Vec<u32>>, start: &str, end: &str) {
    //println!("Turn on");
    let start: Vec<usize> = start.split(',').map(|x| x.parse().unwrap()).collect();
    let end: Vec<usize> = end.split(',').map(|x| x.parse().unwrap()).collect();
    for x in start[0]..=end[0] {
        for y in start[1]..=end[1] {
            grid[x][y] += 1;
        }
    }
}

fn turn_off(grid: &mut Vec<Vec<u32>>, start: &str, end: &str) {
    //println!("Turn off");
    let start: Vec<usize> = start.split(',').map(|x| x.parse().unwrap()).collect();
    let end: Vec<usize> = end.split(',').map(|x| x.parse().unwrap()).collect();
    for x in start[0]..=end[0] {
        for y in start[1]..=end[1] {
            grid[x][y] = grid[x][y].saturating_sub(1);
        }
    }
}

fn toggle(grid: &mut Vec<Vec<u32>>, start: &str, end: &str) {
    //println!("Toggle");
    let start: Vec<usize> = start.split(',').map(|x| x.parse().unwrap()).collect();
    let end: Vec<usize> = end.split(',').map(|x| x.parse().unwrap()).collect();
    for x in start[0]..=end[0] {
        for y in start[1]..=end[1] {
            grid[x][y] += 2;
        }
    }
}

fn count(line: Vec<u32>) -> u32 {
    let mut counter: u32 = 0;
    for light in line {
        counter += light
    }
    counter
}